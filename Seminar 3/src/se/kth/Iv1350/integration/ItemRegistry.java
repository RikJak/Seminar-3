...
public class ItemRegistry {
    ...
    public Item getItem(int itemID) throws ItemNotFoundException,DatabaseFailureException{
        ...
        if(ID.equals(crashDatabase)){
         throw new DatabaseFailureException("DatabaseFailure. Caused by item: " + itemID);
        }
        if(itemRegistry.containsKey(ID)){
		return itemRegistry.get(ID);
        }else{
            throw new ItemNotFoundException("The requested item does not exist in the registry. ItemID: "+ itemID);
        }
